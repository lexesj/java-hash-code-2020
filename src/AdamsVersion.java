...
public class AdamsVersion {
  ...
  public static void main(String[] args) {
    AdamsVersion p = new AdamsVersion();
    p.bruteForce();
  }
    ...
    while (totalTime < numDaysForScanning) {
      for (int i = 0; i < libraries.length; i++) {
        ...
        if (lib != null) {
          ...
          int numOfBooksToSubmit =
              (numDaysForScanning - totalTime - lib.signupLen) * lib.numBooksPerDay;
          ...
          lib.score = sum;
          if (lib.used) lib.score = -1;
        }
      }
      Arrays.sort(
          libraries, Collections.reverseOrder(Comparator.comparing((Library lib) -> lib.score)));
  ...
  class Book implements Comparable<Book> {
    ...
    public int compareTo(Book other) {
      ...
      if (this.bookID == other.bookID) return 0;
      return 1;
    }
  }
